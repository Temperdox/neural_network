...
function setupControls() {
  ...
  document.getElementById('mnist-files').addEventListener('change', (e) => {
    ...
    if (hasTrainImg && hasTrainLbl) {
      ...
      fileStatus.textContent = `${count} file(s) ready to load`;
      fileStatus.className = 'file-status ready';
    } else {
      pendingFiles = null;
      const missing = [];
      if (!hasTrainImg) missing.push('train-images-idx3-ubyte.gz');
      if (!hasTrainLbl) missing.push('train-labels-idx1-ubyte.gz');
...
function updateButtons() {
  ...
  // Load buttons are always available (unless training)
  loadFileBtn.disabled = isTraining;
  loadUrlBtn.disabled = isTraining;
}

// --- Ensure MNIST is loaded (auto or manual) ---
async function ensureMNIST() {
  ...
  setStatus('untrained', 'Please download MNIST files and select them above');
  return false;
}
...
async function startTraining() {
  ...
  const samplesPerEpoch = 60000;
  const batchesPerEpoch = Math.floor(samplesPerEpoch / batchSize);
  let bestTestAcc = 0;
  let lr = learningRate;
  ...
  for (let epoch = 0; epoch < epochs; epoch++) {
    ...
    const trainAcc = epochCorrect / epochTotal * 100;
    console.log(`Epoch ${epoch + 1}/${epochs}: train=${trainAcc.toFixed(1)}%, test=${testAcc ? (testAcc * 100).toFixed(1) : 'N/A'}%, loss=${(epochLoss / batchesPerEpoch).toFixed(4)}`);
  ...
  setStatus('trained', `Model trained (${accStr}% test accuracy)`);
  progressAccuracy.textContent = `Test: ${accStr}%`;
  updateButtons();
  visualizer.draw(network);
}

// --- Save model as a downloadable .json file ---
function saveModelToFile() {
  ...
  modelInfo.textContent = `Model saved as ${a.download} (${(json.length / 1024).toFixed(0)} KB)`;
}

// --- Load model from a local .json file ---
async function loadModelFromFile(file) {
  try {
    modelInfo.textContent = 'Reading model file...';
    const text = await file.text();
    network = NeuralNetwork.deserialize(text);
    modelTrained = true;
    setStatus('trained', 'Model loaded from file');
    modelInfo.textContent = `Loaded: ${file.name} (${(text.length / 1024).toFixed(0)} KB)`;
    updateButtons();
    visualizer.draw(network);
  } catch (e) {
    modelInfo.textContent = 'Failed to load model: ' + e.message;
  }
}

// --- Load model from a URL (e.g. GitHub raw link) ---
async function loadModelFromUrl() {
  const url = prompt('Enter URL to model .json file:\n(e.g. a GitHub raw link)');
  if (!url || !url.trim()) return;

  try {
    modelInfo.textContent = 'Downloading model...';
    ...
    modelInfo.textContent = `Loaded from URL (${(text.length / 1024).toFixed(0)} KB)`;
    updateButtons();
    visualizer.draw(network);
  } catch (e) {
    setStatus('untrained', 'Failed to load model');
